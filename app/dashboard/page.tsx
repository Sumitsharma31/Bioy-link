...
import { Share2, Plus, GripVertical, Pencil, BarChart3, ArrowRight, Sparkles, Globe, ShoppingBag, Podcast, AtSign, Rss } from 'lucide-react';
    ...
    <div className="space-y-xl">
      ...
      <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-md pt-sm md:pt-0">
        <div>
          ...
          <p className="text-body-md text-on-surface-variant mt-xs">Welcome back, {profile.username}. Manage your creator profile and link ecosystem.</p>
        </div>
        <div className="flex gap-md items-center">
          <RefreshButton />
          <Link href={`/${profile.username}`} target="_blank" className="bg-secondary-container text-on-secondary-container px-md py-sm rounded-lg text-label-md flex items-center gap-sm hover:opacity-90 transition-opacity">
            <Share2 size={18} /> View Profile
          </Link>
          ...
          <div key={link.id} className="bg-surface-container-low border border-outline-variant/10 p-md rounded-xl flex flex-col sm:flex-row items-start sm:items-center gap-md hover:border-outline/50 transition-colors">
            ...
            <div className="flex items-center gap-md w-full sm:w-auto justify-between sm:justify-end border-t sm:border-t-0 border-outline-variant/10 pt-sm sm:pt-0">
              ...
              <div className="flex gap-sm">
                <Link href="/dashboard/links" className="p-xs sm:p-sm hover:bg-surface-variant rounded-lg transition-colors"><BarChart3 size={18} className="text-on-surface-variant" /></Link>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="bg-surface-container-low border border-outline-variant/10 p-md sm:p-lg rounded-xl flex flex-col sm:flex-row items-start sm:items-center gap-lg">
        <div className="flex-1">
          ...
          <Link href="/dashboard/appearance" className="inline-block mt-md px-md py-sm border border-outline-variant/30 rounded-lg text-label-md hover:bg-surface-variant transition-colors">Customize Theme</Link>
        </div>
        <div className="flex gap-sm">
          {(() => {
             const themes = [
